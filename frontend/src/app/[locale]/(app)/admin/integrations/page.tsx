...
import { TrendingUp, CheckCircle, Activity, AlertCircle, AlertTriangle, Settings, Eye, Plus, ArrowRight } from 'lucide-react';
            ...
            <main className="flex-1 max-w-[1600px] mx-auto w-full px-6 py-8">
                ...
                <div className="flex flex-wrap justify-between items-end gap-6 mb-8">
                    ...
                    <div className="flex gap-2 p-1 bg-slate-100 dark:bg-slate-800 rounded-lg">
                        <Button variant="ghost" className="px-4 py-2 text-xs font-bold rounded-md bg-white dark:bg-slate-700 shadow-sm text-primary">LIVE STATUS</Button>
                        <Button variant="ghost" className="px-4 py-2 text-xs font-bold text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-white">HISTORICAL DATA</Button>
                    </div>
                </div>

                {/* Metric Cards */}
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-10">
                    <div className="flex flex-col gap-2 rounded-xl p-6 bg-white dark:bg-background border border-slate-200 dark:border-border-dark shadow-sm">
                        ...
                        <p className="text-green-600 text-xs font-bold flex items-center gap-1">
                            <TrendingUp className="text-sm" /> +12% from last month
                        </p>
                    </div>
                    <div className="flex flex-col gap-2 rounded-xl p-6 bg-white dark:bg-background border border-slate-200 dark:border-border-dark shadow-sm">
                        <div className="flex justify-between items-start">
                    ...
                    <div className="flex flex-col gap-2 rounded-xl p-6 bg-white dark:bg-background border border-slate-200 dark:border-border-dark shadow-sm">
                        ...
                        <p className="text-red-500 text-xs font-bold flex items-center gap-1">
                            <AlertTriangle className="text-sm" /> 4 failed requests today
                        </p>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 mb-12">
                    {integrations.map((service) => (
                        <div key={service.id} className="group flex flex-col bg-white dark:bg-background border border-slate-200 dark:border-border-dark rounded-xl overflow-hidden hover:shadow-lg transition-all">
                            ...
                            <div className="p-5 flex flex-col gap-4">
                                ...
                                <div className="pt-4 border-t border-slate-100 dark:border-border-dark flex justify-between gap-2">
                                    <Button variant="primary" className="flex-1 bg-primary/10 text-primary text-xs font-bold py-2 rounded hover:bg-primary hover:text-white transition-colors">Test Connection</Button>
                                    <button type="button" className="px-3 border border-slate-200 dark:border-slate-700 rounded flex items-center text-slate-500 hover:bg-primary/5 dark:hover:bg-violet-500/15">
                    ...
                    <div className="flex flex-col border-2 border-dashed border-slate-300 dark:border-slate-700 rounded-xl p-8 items-center justify-center text-center gap-4 bg-slate-50/50 dark:bg-background/50 hover:bg-primary/8 dark:hover:bg-violet-500/15 transition-all group cursor-pointer">
                ...
                <div className="bg-white dark:bg-background border border-slate-200 dark:border-border-dark rounded-xl shadow-sm overflow-hidden">
                    <div className="p-6 border-b border-slate-100 dark:border-border-dark flex justify-between items-center">
                        ...
                        <button type="button" className="text-primary text-sm font-bold flex items-center gap-1 hover:underline">
                            View All Logs <ArrowRight className="text-sm" />
                        </button>
                    </div>
                    <div className="overflow-x-auto">
                        <table className="w-full text-left text-sm">
                            ...
                            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                                {logs.map((log) => (
                                    <tr key={log.id} className="hover:bg-primary/5 dark:hover:bg-violet-500/8 transition-colors">
