...
export const requireOrganization = async (
  ...
  try {
    ...
    // ────────────────────────────────────────────────────────────────────────
    // NEW: Multi-Tenancy Physical Isolation Logic
    // ────────────────────────────────────────────────────────────────────────
    const orgDoc = await Organization.findById(organizationId).select('+databaseUri');
    if (orgDoc && orgDoc.databaseUri) {
      try {
        // Établir ou récupérer la connexion dédiée
        const tenantConn = await tenantManager.getTenantConnection(organizationId, orgDoc.databaseUri);
        
        // Attacher la connexion à la requête pour usage ultérieur
        (req as any).tenantConn = tenantConn;
        
        logger.debug(`[Multi-Tenancy] 🏭 Switched to dedicated DB for org ${organizationId}`, { requestId: req.id });
      } catch (dbError: any) {
        logger.error(`[Multi-Tenancy] ❌ Failed to switch DB for org ${organizationId}: ${dbError.message}`);
        return next(new AppError('Erreur de connexion au serveur de données dédié du client', 502, 'TENANT_DB_CONNECTION_ERROR'));
      }
    }
