...
async function testProtectedEndpoint(token: string, endpoint: string, organizationId?: string) {
    try {
        ...
        results.push({
            test: `Access ${endpoint}`,
            status: 'FAIL',
            details: error.response?.status === 401 ? 'Unauthorized (401)' :
                error.response?.status === 400 ? `Bad Request (400): ${JSON.stringify(error.response.data)}` :
                    error.response?.status === 403 ? 'Forbidden (403)' : error.message
        });
        return null;
    }
}
