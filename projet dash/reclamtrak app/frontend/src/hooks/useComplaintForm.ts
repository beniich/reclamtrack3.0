...
export function useComplaintForm(options: UseComplaintFormOptions = {}) {
    ...
    const submitComplaint = useCallback(async () => {
        ...
        try {
            ...
            // 1. Écriture immédiate dans la DB locale (Vault)
            await db.transaction('rw', db.complaints, db.outbox, async () => {
                await db.complaints.add({
                    localId,
                    ...rawData,
                    photos: [], // On pourrait stocker les base64 ici si besoin
                    syncState: 'pending',
                    createdAt: new Date(),
                    updatedAt: new Date(),
                } as any);

                // 2. Ajouter l'événement à l'Outbox pour la synchro en arrière-plan
                await db.outbox.add({
            ...
            // Succès local immédiat !
            clearDraft();
            toast.success('Réclamation enregistrée localement !', {
            ...
            toast.error('Erreur lors de l\'enregistrement local', {
                description: 'Vos données ont été conservées, nous réessaierons plus tard.',
            });
        } finally {
