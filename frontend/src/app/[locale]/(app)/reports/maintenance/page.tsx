...
export default function MaintenanceReportsPage() {
    ...
    const kpis = [
        { label: 'OEE (TRS) Global', value: '82.4%', trend: '+2.1%', up: true, icon: Target, color: 'text-primary' },
        { label: 'Disponibilité', value: '91.2%', trend: '-0.5%', up: false, icon: Zap, color: 'text-emerald-500' },
        { label: 'MTBF (Fiabilité)', value: '420h', trend: '+15h', up: true, icon: Activity, color: 'text-primary' },
        { label: 'MTTR (Réparabilité)', value: '2.4h', trend: '-12min', up: true, icon: Clock, color: 'text-primary' },
    ];

    return (
        <div className="p-6 space-y-8 bg-slate-50 dark:bg-background min-h-[calc(100vh-6rem)] relative overflow-hidden">
            {/* Ambient Background Glows */}
            <div className="absolute top-0 right-0 w-[600px] h-[600px] bg-primary/5 dark:bg-primary/10 blur-[120px] -z-10 pointer-events-none rounded-full"></div>
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 relative z-10">
                <div className="flex items-center gap-3">
                    <div className="p-3 bg-primary dark:bg-primary text-white rounded-2xl shadow-lg shadow-indigo-500/30">
                        <BarChart3 className="w-6 h-6" />
                    </div>
                    <div>
                        <h2 className="text-3xl font-black text-slate-900 dark:text-white uppercase italic tracking-tighter">
                            Analytiques <span className="text-primary dark:text-primary not-italic">Industriels</span>
                        </h2>
                ...
                <div className="flex gap-3">
                    ...
                    <button type="button" className="h-[52px] bg-surface-dark dark:bg-white text-white dark:text-slate-900 rounded-2xl px-6 text-[10px] font-black uppercase tracking-widest flex items-center gap-2 shadow-xl hover:scale-105 transition-all">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6 relative z-10">
                {kpis.map((kpi, i) => (
                    <div key={i} className="bg-white dark:bg-background border border-slate-200 dark:border-border-dark rounded-[2rem] p-6 shadow-sm group hover:shadow-xl hover:shadow-indigo-500/5 transition-all animate-in zoom-in-95 duration-500" style={{ animationDelay: `${i * 100}ms` }}>
                        <div className="flex items-start justify-between mb-4">
                            <div className={`p-3 rounded-2xl bg-slate-50 dark:bg-slate-800 transition-colors group-hover:bg-indigo-50 dark:group-hover:bg-primary/10`}>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 relative z-10">
                ...
                <div className="lg:col-span-2 bg-white dark:bg-background border border-slate-200 dark:border-border-dark rounded-[2.5rem] p-8 shadow-sm flex flex-col min-h-[400px]">
                    <div className="flex items-center justify-between mb-8">
                        <div>
                            <h3 className="text-sm font-black uppercase italic tracking-tighter text-slate-900 dark:text-white flex items-center gap-2">
                                <Activity className="w-4 h-4 text-primary" /> Taux de Panne (Courbe de Baignoire)
                            </h3>
                        ...
                        <div className="flex gap-2">
                            {['Hebdo', 'Mensuel', 'Trim'].map((t) => (
                                <button type="button" key={t} className={`px-3 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all ${t === 'Mensuel' ? 'bg-primary text-white shadow-lg' : 'bg-slate-100 dark:bg-slate-800 text-slate-500 hover:bg-slate-200'}`}>
                                    {t}
                                </button>
                            ))}
                        </div>
                ...
                <div className="space-y-6">
                    {/* OEE Decomposition */}
                    <div className="bg-surface-dark text-white rounded-[2.5rem] p-8 shadow-2xl relative overflow-hidden group">
                        ...
                        <div className="space-y-6">
                            {[
                                { label: 'Performance', val: 94.5, color: 'bg-primary' },
                                { label: 'Disponibilité', val: 91.2, color: 'bg-emerald-500' },
                                { label: 'Qualité', val: 98.9, color: 'bg-primary' },
                            ].map((item, idx) => (
                                <div key={idx}>
                    ...
                    <div className="bg-white dark:bg-background border border-slate-200 dark:border-border-dark rounded-[2rem] p-8 shadow-sm">
                        <h3 className="text-[11px] font-black uppercase tracking-widest text-slate-400 mb-6 flex items-center gap-2">
                            <AlertTriangle className="w-3.5 h-3.5 text-primary" /> Causes de Pannnes
                        </h3>
                        <div className="space-y-4">
                            {[
                                ...
                                <div key={idx} className="flex items-center justify-between border-b border-slate-100 dark:border-border-dark pb-3 last:border-0 last:pb-0">
