...
export default function MROInventoryPage() {
    ...
    return (
        <div className="p-6 space-y-8 bg-slate-50 dark:bg-background min-h-[calc(100vh-6rem)] relative overflow-hidden">
            <div className="absolute top-1/3 left-1/4 w-[400px] h-[400px] bg-primary/5 dark:bg-primary/10 blur-[100px] -z-10 pointer-events-none rounded-full"></div>
            
            {/* Header */}
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 relative z-10">
                <div className="flex items-center gap-3">
                    <div className="p-3 bg-primary dark:bg-primary text-white rounded-2xl shadow-lg shadow-blue-500/30 dark:shadow-blue-500/20">
                    ...
                    <div>
                        <h2 className="text-3xl font-black text-slate-900 dark:text-white uppercase italic tracking-tighter">
                            Magasin <span className="text-primary dark:text-primary not-italic">MRO</span>
                        </h2>
                ...
                <div className="flex gap-3">
                    <button type="button" className="h-[52px] px-6 rounded-2xl bg-white dark:bg-background border border-slate-200 dark:border-border-dark text-slate-700 dark:text-slate-300 text-[10px] font-black uppercase tracking-widest hover:border-blue-300 dark:hover:border-primary/50 flex items-center gap-2 transition-all shadow-sm">
                        <Truck className="w-4 h-4" /> Réapprovisionner
                    </button>
                    <button type="button" className="h-[52px] bg-primary dark:bg-primary hover:brightness-110 text-white rounded-2xl px-6 text-[10px] font-black uppercase tracking-widest flex items-center gap-2 shadow-xl shadow-blue-600/20 dark:shadow-blue-600/20 transition-all">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 relative z-10">
                <div className="bg-white dark:bg-background border border-slate-200 dark:border-border-dark rounded-[1.5rem] p-6 shadow-sm flex items-center gap-4">
                ...
                <div className="bg-white dark:bg-background border border-slate-200 dark:border-border-dark rounded-[1.5rem] p-6 shadow-sm flex items-center gap-4 relative overflow-hidden group">
            ...
            <div className="flex flex-col lg:flex-row gap-4 relative z-10">
                <div className="flex-1 relative">
                    ...
                    <input 
                        ...
                        className="w-full h-[52px] pl-12 pr-4 rounded-2xl bg-white dark:bg-background border border-slate-200 dark:border-border-dark text-[12px] font-medium text-slate-900 dark:text-white placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-primary/20 transition-all"
                    />
                </div>
                <button type="button" className="h-[52px] px-6 flex items-center justify-center bg-white dark:bg-background border border-slate-200 dark:border-border-dark rounded-2xl hover:bg-primary/5 dark:hover:bg-violet-500/15 transition-colors text-[10px] font-black uppercase tracking-widest text-slate-700 dark:text-slate-300 gap-2">
                    <Filter className="w-4 h-4 text-slate-400" /> Filtrer
                </button>
            </div>

            {/* List */}
            <div className="bg-white dark:bg-background border border-slate-200 dark:border-border-dark rounded-[2rem] shadow-sm relative z-10 overflow-hidden">
                <div className="overflow-x-auto">
                    <table className="w-full text-left">
                        <thead>
                            <tr className="border-b border-slate-100 dark:border-border-dark bg-slate-50 dark:bg-background/50">
                        ...
                        <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                            {mockMroItems.map((item) => {
                                ...
                                return (
                                    <tr key={item.id} className="group hover:bg-primary/5 dark:hover:bg-violet-500/8 transition-colors">
                                        ...
                                        <td className="px-6 py-5 cursor-pointer">
                                            <Link href={`/inventory/mro/${item.id}`}>
                                                <p className="text-sm font-black italic uppercase tracking-tighter text-slate-900 dark:text-white group-hover:text-primary transition-colors">{item.name}</p>
                                        ...
                                        <td className="px-6 py-5 text-right">
                                            <Link href={`/inventory/mro/${item.id}`}>
                                                <button type="button" className="size-10 rounded-xl bg-white dark:bg-background border border-slate-200 dark:border-border-dark flex items-center justify-center text-slate-400 hover:text-primary hover:border-blue-300 transition-colors shadow-sm">
