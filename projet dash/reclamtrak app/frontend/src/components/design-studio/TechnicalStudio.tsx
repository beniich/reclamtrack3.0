...
export const TechnicalStudio: React.FC = () => {
    ...
    const handleCanvaEdit = async () => {
        ...
        try {
            ...
            const result = await canvaService.editSvg(svg);
            
            if (result?.svg) {
                project.clear();
                project.importSVG(result.svg);
            }
        } catch (error) {
            console.error("Canva Error:", error);
            alert("Erreur lors de l'édition avec Canva.");
        } finally {
    ...
    const handleSave = async () => {
        ...
        const name = window.prompt(t('namingPrompt'));
        if (!name) return;
    ...
    const handleSelectItem = (info: any) => {
        ...
        setSelectedAsset({
            ...
            status: isPump ? t('maintenance') : t('operational'),
            lastMaintenance: isPump ? '12 Mar 2026' : '01 Jan 2026',
            ...
            <div className="flex-1 flex flex-col gap-4 relative">
                ...
                <div className="absolute top-4 right-4 z-10 flex gap-2">
                    ...
                    >
                        ...
                        <span className="text-xs font-bold uppercase tracking-widest px-2 border-l border-white/10">{t('archives')}</span>
                    </button>

                    <button
                        onClick={handleCanvaEdit}
                        disabled={isCanvaLoading || !project}
                        className="flex items-center gap-2 bg-gradient-to-r from-[#00c4cc] to-[#7d2ae8] hover:brightness-110 border border-white/20 text-white px-4 py-2 rounded-xl shadow-lg shadow-purple-500/20 backdrop-blur-md transition-all disabled:opacity-50"
                    >
                        {isCanvaLoading ? <RefreshCw size={16} className="animate-spin" /> : <Palette size={16} />}
                        <span className="text-xs font-bold uppercase tracking-widest px-2 border-l border-white/10">Canva</span>
                    </button>
                    ...
                    >
                        ...
                        <span className="text-xs font-bold uppercase tracking-widest px-2 border-l border-white/10">{t('save')}</span>
                    </button>
                </div>
                        ...
                        <div className="w-80 bg-[#1A0536] border-l border-white/10 h-full flex flex-col animate-in slide-in-from-right duration-300">
                            <div className="p-6 border-b border-white/10 flex items-center justify-between bg-[#310B5E]/30">
                                <div className="flex items-center gap-2">
                                    ...
                                    <h3 className="text-sm font-black text-white uppercase tracking-tighter">{t('archivedSchemes')}</h3>
                                </div>
                            ...
                            <div className="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar">
                                {isLoading ? (
                                    ...
                                    <div className="text-center py-10 opacity-30">
                                        <p className="text-xs font-bold uppercase text-white">{t('noSchemeFound')}</p>
                                    </div>
                                ) : (
                                    schemas.map((s) => (
                                        <button
                ...
                <div className="flex-1 min-h-0 bg-white dark:bg-slate-900 rounded-[2.5rem] overflow-hidden border border-slate-200 dark:border-orange-500/10 shadow-inner flex relative">
                    ...
                    {selectedAsset && (
                        <div className="absolute top-4 right-4 bottom-4 w-72 bg-white/95 dark:bg-slate-950/95 backdrop-blur-xl border border-slate-200 dark:border-white/10 rounded-[2rem] shadow-2xl p-6 flex flex-col animate-in slide-in-from-right duration-300 z-20">
                            ...
                            <div className="mb-6">
                                <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-1">{t('assetId')}</p>
                            ...
                            <div className="space-y-4 flex-1">
                                <div className="p-4 bg-slate-50 dark:bg-white/5 rounded-2xl border border-slate-100 dark:border-white/5">
                                    <div className="flex items-center justify-between mb-2">
                                        <span className="text-[9px] font-black uppercase text-slate-400 tracking-widest">{t('healthState')}</span>
                                        <span className={`px-2 py-0.5 rounded-full text-[8px] font-black uppercase tracking-widest ${selectedAsset.status === t('operational') ? 'bg-emerald-500/10 text-emerald-500' : 'bg-amber-500/10 text-amber-500'}`}>
                                            {selectedAsset.status}
                                        </span>
                                    </div>
                                    <div className="flex items-center gap-2">
                                        <ShieldCheck size={14} className={selectedAsset.status === t('operational') ? 'text-emerald-500' : 'text-amber-500'} />
                                        <span className="text-xs font-bold text-slate-700 dark:text-slate-300">{t('diagnosticOk')}</span>
                                    </div>
                                </div>

                                <div className="grid grid-cols-2 gap-3">
                                    <div className="p-3 bg-slate-50 dark:bg-white/5 rounded-xl border border-slate-100 dark:border-white/5">
                                        <p className="text-[8px] font-black text-slate-400 uppercase mb-1">{t('lastPM')}</p>
                                        <p className="text-[10px] font-bold text-slate-700 dark:text-slate-300">{selectedAsset.lastMaintenance}</p>
                                    </div>
                                    <div className="p-3 bg-slate-50 dark:bg-white/5 rounded-xl border border-slate-100 dark:border-white/5">
                                        <p className="text-[8px] font-black text-slate-400 uppercase mb-1">{t('mtbf')}</p>
