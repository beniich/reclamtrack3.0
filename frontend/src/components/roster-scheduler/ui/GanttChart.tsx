...
interface GanttChartProps {
    onCellClick?: (teamName: string, timeIndex: number) => void;
}

export default function GanttChart({ onCellClick }: GanttChartProps) {
    return (
        <div className="flex-1 relative overflow-auto scrollbar-thin scrollbar-thumb-slate-800 bg-background-dark/20">
            {/* Time Header */}
            <div className="sticky top-0 z-40 flex bg-surface-dark border-b border-slate-700 shadow-xl w-fit min-w-full">
                <div className="w-48 shrink-0 border-r border-slate-700 p-4 text-xs font-bold uppercase text-slate-500 bg-surface-dark sticky left-0 z-50">
                    Teams / Assets
                </div>
                <div className="flex flex-1">
                    {['08:00', '09:00', '10:00', '11:00', '12:00', '13:00', '14:00', '15:00', '16:00', '17:00', '18:00'].map((time) => (
                        <div
                            key={time}
                            className={`w-32 flex-none border-r border-border-dark p-4 text-center text-[11px] font-bold ${time === '10:00' ? 'text-primary bg-primary/5' : 'text-slate-500'}`}
                        >
                            {time}
                        </div>
                    ))}
            ...
            <div className="relative min-h-full w-fit min-w-full">
                ...
                <div className="absolute top-0 bottom-0 left-[304px] w-px bg-primary z-30 shadow-[0_0_8px_rgba(59,130,246,0.5)] pointer-events-none">
                ...
                {teamsData.map((row, idx) => (
                    <div
                        key={idx}
                        className="flex border-b border-border-dark/50 group h-24 hover:bg-violet-500/15/20 transition-colors w-fit min-w-full"
                    >
                        <div className="w-48 shrink-0 border-r border-slate-700 p-4 sticky left-0 bg-surface-dark z-10 flex flex-col justify-center group-hover:bg-violet-500/15 transition-colors">
                        ...
                        <div className="flex flex-1 relative">
                            ...
                            {[...Array(11)].map((_, i) => (
                                <div 
                                    ...
                                    className={`w-32 h-full border-r border-border-dark/30 flex-none cursor-pointer hover:bg-violet-500/15/40 transition-colors ${i === 2 ? 'bg-primary/5' : ''}`}
                                ></div>
                            ))}
                        </div>
                    </div>
