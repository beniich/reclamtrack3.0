...
export default function AssetsRegistryPage() {
    ...
    const getClassificationColor = (cls: string) => {
        switch(cls) {
            ...
            case 'CONFIDENTIAL': return 'bg-primary/10 text-primary border-primary/20';
    ...
    return (
        <div className="p-6 space-y-8 bg-slate-50 dark:bg-background min-h-screen relative overflow-hidden">
            {/* Ambient glow */}
            <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-primary/5 dark:bg-primary/5 blur-[120px] -z-10 pointer-events-none rounded-full"></div>
            
            {/* Header Section */}
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 relative z-10">
                <div className="flex items-center gap-3">
                    <div className="p-3 bg-primary dark:bg-primary text-white rounded-2xl shadow-lg shadow-indigo-500/30 dark:shadow-orange-500/20">
                        <Box className="w-6 h-6" />
                    </div>
                    <div>
                        <h2 className="text-3xl font-black text-slate-900 dark:text-white uppercase italic tracking-tighter">
                            Registre <span className="text-primary dark:text-primary not-italic">Équipements</span>
                        </h2>
                ...
                <div className="flex gap-3">
                    ...
                    <button type="button" className="h-[52px] bg-primary dark:bg-primary hover:brightness-110 text-white rounded-2xl px-6 text-[10px] font-black uppercase tracking-widest flex items-center gap-2 shadow-xl shadow-indigo-600/20 dark:shadow-orange-600/20 transition-all">
            ...
            <div className="flex flex-col lg:flex-row gap-4">
                <div className="flex-1 relative">
                    ...
                    <input 
                        ...
                        className="w-full h-[52px] pl-12 pr-4 rounded-2xl bg-white dark:bg-background border border-slate-200 dark:border-border-dark text-[12px] font-medium text-slate-900 dark:text-white placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-primary/20 dark:focus:ring-primary/20 transition-all"
                    />
                </div>
                <div className="flex gap-4">
                    <Select defaultValue="all">
                        <SelectTrigger className="w-[180px] h-[52px] bg-white dark:bg-background border-slate-200 dark:border-border-dark rounded-2xl text-[10px] font-black uppercase tracking-widest">
                            <SelectValue placeholder="Catégorie" />
                        </SelectTrigger>
                        <SelectContent className="bg-white dark:bg-background border-slate-200 dark:border-border-dark">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 relative z-10">
                {mockAssets.map((asset, i) => (
                    <Link href={`/assets/${asset.id}`} key={asset.id}>
                        <div className="bg-white dark:bg-background rounded-[2rem] border border-slate-200 dark:border-border-dark/80 p-6 hover:shadow-xl hover:shadow-indigo-500/5 dark:hover:border-primary/30 transition-all group cursor-pointer animate-in zoom-in-95 duration-500" style={{ animationDelay: `${i * 100}ms` }}>
                            ...
                            <h3 className="text-lg font-black text-slate-900 dark:text-white uppercase italic tracking-tight mb-1 group-hover:text-primary dark:group-hover:text-orange-400 transition-colors">
                                {asset.name}
                            </h3>
                                ...
                                <div className="flex items-center gap-4">
                                    ...
                                    <div className="flex flex-col">
                                        ...
                                        <div className="flex items-center gap-1.5">
                                            ...
                                            {asset.hasGeo && <MapPin className="w-3 h-3 text-primary dark:text-primary" />}
                                        </div>
                                    </div>
                                </div>
                                <div className="size-8 rounded-xl bg-slate-50 dark:bg-slate-800 flex items-center justify-center group-hover:bg-primary dark:group-hover:bg-primary group-hover:text-white transition-all">
